...
3. First determine edit_intent from the user's request:
   - "add" (default) — install new equipment into the scene, leaving everything else untouched.
   - "replace" — the request asks to swap out existing equipment (e.g. "replace the old wall AC with a mini-split", "swap the furnace for a heat pump"). The enriched_prompt must FIRST remove the existing unit, THEN install the new one following the installation rules.
   - "remove" — the request only asks to take equipment out (e.g. "remove the radiator"). The enriched_prompt removes it and realistically restores the wall/floor/surface behind it; do NOT add anything.
4. enriched_prompt must be a single, self-contained image-editing instruction that:
   - Begins with an action verb ("Add"/"Install"/"Mount"/"Place" for add; "Remove the existing … and install …" for replace; "Remove … and restore …" for remove)
   ...
   - Preserves the rest of the photo exactly — only the equipment (and its removal, for replace/remove) changes; do not restyle, recolor, reframe, or alter anything else in the scene
   - Maximum 200 words
5. request_viable is false when the photo makes a REALISTIC installation impossible — e.g. no suitable wall space, the space is entirely obstructed, OR the equipment's required environment doesn't match the photo (an outdoor condenser requested for an indoor room, or an indoor unit requested for an outdoor-only shot). Do not force an unrealistic placement just to produce an image.
6. viability_reason is required even when request_viable is true (state why it is viable in one sentence).
7. content_flag classifies the REQUEST + PHOTO for content safety:
...
8. detected_category is your best classification of the requested equipment into exactly one of these values (grouped for reference):
   ...
   - Fallback: other
   Base it on the user's request (and equipment context if given). For "replace" this is the NEW equipment; for "remove" it is the equipment being removed. Use "other" only if it genuinely fits none.
9. schema_version is always exactly "2.3".
...
REQUIRED OUTPUT SCHEMA:
{
  ...
  "edit_intent": "<exactly one of: add | replace | remove>",
  "request_viable": <boolean>,
  "viability_reason": "<one sentence>",
  "enriched_prompt": "<detailed image-editing instruction, max 200 words, starting with action verb, obeying the installation realism rules, changing only the equipment>",
  ...
  "detected_category": "<one of the equipment category values listed in rule 8>",
  "schema_version": "2.3"
}`;
